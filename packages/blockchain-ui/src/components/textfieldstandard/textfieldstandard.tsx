...
export interface ITextFieldStandardProps extends InputHTMLPropsWithoutPrefix {
  state: string;
  error?: boolean;
  disabled?: boolean;
  types?: string;
...
const TextInputField = styled.input<{ error?: boolean; types: string; background?: boolean; label?: boolean }>`
  ...
  padding-left: ${({ label }) => (label ? "6px" : "14px")};
...
export const TextFieldStandard: FunctionComponent<ITextFieldStandardProps> = ({ state, prefix, background, label }) => {
